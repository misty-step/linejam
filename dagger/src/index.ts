...
export class Ci {
  ...
  async audit(source: Directory): Promise<string> {
    // The parser contract is pinned to this scanner's JSON schema and exit
    // semantics. Reuse its static release binary without installing app deps.
    const scanner = dag
      .container()
    ...
    return dag
      .container()
      .from(NODE_IMAGE)
      .withFile('/usr/local/bin/osv-scanner', scanner)
      .withMountedDirectory('/src', source)
      .withWorkdir('/src')
      .withExec(['node', 'scripts/ci/osv-audit.mjs'])
      .stdout();
  }

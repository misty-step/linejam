...
(function () {
  ...
  window.LANE_SPECS['ANTHRO-3'] = {
    ...
    css: `
      ...
      .opt-ANTHRO-3 .readpoem .verse{ padding:14px 0; }
      .opt-ANTHRO-3 .readpoem .vt{ font:400 32px/1.28 'Fraunces',serif; color:var(--cream); }
      ...
      .opt-ANTHRO-3 .readpoem .veiled{ opacity:.42; }
      .opt-ANTHRO-3 .readpoem .veilbar{ height:11px; background:var(--line); border-radius:3px; }
      .opt-ANTHRO-3 .readpoem .live .vt{ animation:a3up .5s ease both; }

      @media (prefers-reduced-motion: reduce){
        .opt-ANTHRO-3 .glow{ animation:none; }
        .opt-ANTHRO-3 .spotlit .verse{ animation:none; }
        .opt-ANTHRO-3 .readpoem .live .vt{ animation:none; }
    ...
    screens: {
      ...
      reveal(el, corpus) {
        ...
        el.innerHTML = `
          <div class="a3">
            <div class="a3-body">
              <p class="now">the running order</p>
              <ul class="cast" style="margin-top:14px">
                ${corpus.revealQueue
                  .map(
                    (q) => `<li>
                      <span class="seat ${q.reader === first.reader ? 'here' : ''}">${q.reader[0]}</span>
                      <span class="cname">${q.reader}</span>
                      <span class="crole">reads ${q.poem}</span>
                    </li>`
                  )
                  .join('')}
              </ul>
              <hr class="rule-g" style="margin-top:22px" />
              <p class="now" style="margin-top:4px">first up &middot; now reading</p>
            ...
            <div class="a3-foot">
              <button class="btn btn-gold">Begin the reading</button>
            </div>
          </div>`;
      },
      read(el, corpus) {
        ...
        const lines = p.lines
          .map((l, i) => {
            const shown = i < r.revealedLines;
            const live = i === r.revealedLines - 1;
            if (!shown) {
              return `<div class="verse veiled"><div class="veilbar" style="width:${Math.min(words(l.text), 5) * 34 + 26}px"></div></div>`;
            }
            return `<div class="verse ${live ? 'live' : ''}">
              <div class="vt">${l.text}</div>
              <div class="vby">${l.author}</div>
            </div>`;
          })
        ...
        el.innerHTML = `
          <div class="a3">
            <div class="a3-body">
              <p class="now">the reading &middot; poem ${r.position} of ${r.total}</p>
              <h2 class="reader">${r.reader}</h2>
              <p class="reads">reads ${p.title} aloud, one line at a time</p>
              <div class="prog"><span class="prog-fill" style="width:${(r.revealedLines / total) * 100}%"></span></div>
              <p class="prog-n">line ${r.revealedLines} of ${total} revealed</p>
              <div class="readpoem">${lines}</div>
            </div>
            <div class="a3-foot">
              <button class="btn btn-gold">Reveal the next line</button>
              <p class="bill" style="text-align:center;color:var(--dim);margin-top:12px">then hand to ${r.nextReader}</p>
            </div>
          </div>`;
      },
    },
  ...
  window.LANE_SPECS['ANTHRO-4'] = {
    ...
    css: `
      ...
      .opt-ANTHRO-4 .v4.veiled{ opacity:.45; }
      .opt-ANTHRO-4 .v4.veiled .v4-u{ background:var(--line); }
      ...
      reveal(el, corpus) {
        ...
        el.innerHTML = `
          <div class="a4">
            ...
            <div class="a4-foot">
              <button class="btn btn-fill">Begin the reading</button>
            </div>
          </div>`;
      },
      read(el, corpus) {
        ...
        const lines = p.lines
          .map((l, i) => {
            ...
            const w = words(l.text);
            if (!shown) {
              return `<div class="v4 veiled">
                <div class="v4-row"><span class="v4-t">&nbsp;</span></div>
                <div class="v4-u" style="width:${(w / max) * 100}%"></div>
              </div>`;
            }
            return `<div class="v4">
          ...
          <div class="a4">
            <div class="a4-body">
              <div class="read-head">
                ...
                <p class="given-l">${p.title}, aloud</p>
              </div>
              <div class="prog4"><span style="width:${(r.revealedLines / total) * 100}%"></span></div>
              <p class="prog4-n">line ${r.revealedLines} of ${total} revealed</p>
              <div class="vstack">${lines}</div>
            </div>
            <div class="a4-foot">
              <button class="btn btn-fill">Reveal the next line</button>
              <p class="kick" style="text-align:center;margin-top:12px">then hand to ${r.nextReader}</p>
            </div>
          </div>`;
      },
    },
  ...
  window.LANE_SPECS['ANTHRO-5'] = {
    ...
    css: `
      ...
      .opt-ANTHRO-5 .v5.veiled{ opacity:.4; }
      .opt-ANTHRO-5 .v5.veiled .v5-ct i{ background:var(--line); }
    ...
    screens: {
      ...
      reveal(el, corpus) {
        ...
        el.innerHTML = `
          <div class="a5">
            <div class="a5-body">
              <p class="mono red">the proof &middot; read in order</p>
              <ul class="order">
                ${corpus.revealQueue
                  .map(
                    (q, i) => `<li><span class="on">${String(i + 1).padStart(2, '0')}</span>
                      <span class="ot"><b>${q.reader}</b> reads ${q.poem}</span></li>`
                  )
                  .join('')}
              </ul>
              <p class="proof-t">${p.title}</p>
              <div class="vs proof">
                ${p.lines
                  .map((l) => {
                    const w = words(l.text);
            ...
            <div class="a5-foot">
              <button class="btn btn-red">Begin the reading</button>
            </div>
          </div>`;
      },
      read(el, corpus) {
        ...
        const lines = p.lines
          .map((l, i) => {
            ...
            const bars = Array.from({ length: w }).map(() => `<i></i>`).join('');
            if (!shown) {
              return `<div class="v5 veiled">
                <div class="v5-row"><span class="v5-t">&nbsp;</span></div>
                <div class="v5-ct">${bars}</div>
              </div>`;
            }
            return `<div class="v5">
        ...
        el.innerHTML = `
          <div class="a5">
            <div class="a5-body">
              ...
              <p class="mono mut">reads ${p.title} aloud</p>
              <div class="prog5"><span style="width:${(r.revealedLines / total) * 100}%"></span></div>
              <p class="prog5-n">line ${r.revealedLines} of ${total} pulled</p>
              <div class="vs">${lines}</div>
            </div>
            <div class="a5-foot">
              <button class="btn btn-red">Reveal the next line</button>
              <p class="mono mut" style="text-align:center;margin-top:12px">then hand to ${r.nextReader}</p>
            </div>
          </div>`;
      },
    },

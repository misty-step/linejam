...
- Hosted `pnpm build` now bootstraps `GUEST_TOKEN_SECRET` plus `CLERK_JWT_ISSUER_DOMAIN` into the active Convex production or preview target before `convex deploy`, so new preview branches inherit the auth invariants instead of failing on first deploy.
...
- Remote smoke covers both guest and signed-in join flows. Set `PLAYWRIGHT_BASE_URL`, keep `CLERK_SECRET_KEY` and `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` available for the authenticated Clerk path, and use `PLAYWRIGHT_CLERK_TEST_EMAIL` only when you want to override the default provisioned test user. For smoke runs, `scripts/ci/dagger-call.sh` now prefers `.env.production.local` over `.env.local` so deployed targets do not accidentally inherit localhost Clerk keys, and it validates that Clerk already has the `convex` JWT template before launching authenticated smoke. Set `PLAYWRIGHT_REQUIRE_AUTH_SMOKE=0` only when you intentionally want to skip auth smoke.

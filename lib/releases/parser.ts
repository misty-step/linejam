  ...
  for (const line of lines) {
    ...
    if (versionMatch) {
      ...
      if (version.toLowerCase() !== 'unreleased') {
        if (!isReleaseVersion(version) || !isReleaseDate(date)) {
          throw new Error(`Invalid release heading: ${trimmed}`);
        }
      }
      currentRelease = { version, date, changes: [] };
      if (versionMatch[2]) {
        currentRelease.compareUrl = versionMatch[2];
      }
      currentSection = null;
      continue;
    }

...
function configuredUrl(raw, name, acceptedHost) {
  ...
  try {
    ...
    return {
      status: 'fail',
      name,
      message: name + ' must be a valid configured URL',
    };
  }
}
...
export function checkClerkConfig(env = process.env) {
  ...
  if (!publishableKey || !secretKey) {
    ...
    return {
      status: 'fail',
      message: 'missing Clerk values: ' + missing.join(', '),
    };
  }
  if (!/^sk_(?:test|live)_/u.test(secretKey))
    return {
      status: 'fail',
  ...
  return origin
    ...
    : {
        status: 'fail',
        message:
          'NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY does not identify a Clerk Frontend API',
      };
}

/** @param {Env} [env] */
export function checkSentryConfig(env = process.env) {
  if (value(env, 'NEXT_PUBLIC_SENTRY_ENABLED') !== '1') {
    return {
      status: 'fail',
      name: 'Sentry',
  ...
  return configuredUrl(
    value(env, 'NEXT_PUBLIC_SENTRY_DSN'),
    'Sentry',
...
export async function checkAppHealth({
  ...
  try {
    ...
    return {
      ...
      message:
        'no app response at ' +
        url +
        ' (start pnpm dev and run pnpm run doctor again)',
    };
  }
  if (!response.ok)
    return {
  ...
  try {
    ...
    return {
      name: 'app health',
      status: 'fail',
      message: 'app health response was not valid JSON',
    };
  }
  return body?.status === 'ok'
    ...
    : {
        ...
        status: 'fail',
        message:
          'app health reported status ' + String(body?.status ?? 'missing'),
      };
}
...
async function main() {
  ...
  for (const result of results)
    console.log(
      icons[result.status] + ' ' + result.name + ': ' + result.message
    );
  ...
  if (failures.length > 0) {
    console.error(
      '\ndoctor failed ' +
        failures.length +
        ' check(s). Fix the reported configuration and run pnpm run doctor again.'
    );
    process.exitCode = 1;
    return;
  }

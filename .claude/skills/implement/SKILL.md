...
1. Explicit path argument (`/implement backlog.d/010-preview-lane.md`)
2. Backlog ID (`/implement 010`) → resolves via `backlog.d/<id>-*.md`
...
2. **Red.** Write one failing test for one behavior. Pick the layer:
   ...
   - **Convex queries/mutations/actions** → Vitest with current mocking pattern
     ...
     import.meta.glob blocker). Backlog slice 014 formalizes the migration —
     until it lands, match the surrounding file's approach.
...
- Tests follow the existing file's pattern. `tests/convex/*.test.ts` uses
  ...
  explicitly says so — that's slice 014's job.

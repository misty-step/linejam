...
questions about intent. The repo-brief at `.claude/repo-brief.md` is the shared spine —
...
3. Every invariant in `.claude/repo-brief.md` §Invariants is either upheld or has an explicit call-out + justification.
...
1. `.claude/repo-brief.md` — vocabulary, invariants, known debts, gotchas.
...
are the only subagents that exist in `/Users/phaedrus/Development/spellbook/agents/`:

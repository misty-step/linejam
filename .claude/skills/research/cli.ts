...
async function main(): Promise<void> {
  ...
  const configDir =
    process.env.PI_CONFIG_DIR ?? path.resolve(process.cwd(), '..', '..');
  ...
  const useContext7 =
    Boolean(process.env.CONTEXT7_API_KEY) &&
    isDocsLookup(input.query, input.command);
  if (useContext7) {
  ...
  if (
    input.command === 'web-deep' &&
    process.env.PERPLEXITY_API_KEY &&
    results.length > 0
  ) {
    const synthesizer = new PerplexitySynthesisProvider(
      process.env.PERPLEXITY_API_KEY
    );
...
function parseArgs(args: string[]): CliInput {
  if (args.length < 2) {
    throw new Error(
      'usage: web-search <web|web-deep|web-news|web-docs> <query>'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'command must be one of: web, web-deep, web-news, web-docs'
    );
  }

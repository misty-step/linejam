...
describe('query-utils', () => {
  test('normalizeQuery trims, lowers, and collapses spaces', () => {
    expect(normalizeQuery('  Latest   Next.js   docs  ')).toBe(
      'latest next.js docs'
    );
  });
  ...
  test('isTimeSensitiveQuery detects recency intents', () => {
    ...
    expect(isTimeSensitiveQuery('postgres indexing guide', 'web-news')).toBe(
      true
    );
  ...
  test('inferRecencyDays applies explicit value and defaults', () => {
    expect(inferRecencyDays({ query: 'latest ai news', command: 'web' })).toBe(
      30
    );
    expect(
      inferRecencyDays({ query: 'new release', command: 'web-news' })
    ).toBe(7);
    expect(
      inferRecencyDays({ query: 'evergreen topic', command: 'web' })
    ).toBeNull();
    expect(
      inferRecencyDays({
        query: 'x',
  ...
  test('canonicalizeUrl strips tracking params and dedupes', () => {
    ...
    expect(canonicalizeUrl(inputs[0].url)).toBe(

...
- Unresolved Sentry issues are polled from the operator workstation. Each new
  ...
  state; GitHub Issues remains the work ledger for planned work.
...
`~/Development/scripts/sentry-agent-loop.mjs` runs every five minutes as the
Hermes `sentry-agent-loop` `--no-agent` cron job. Empty polls produce no
output.

Each tick:

1. lists unresolved Sentry issues for every mapped project
   (`sentry issue list <org>/<project> --query is:unresolved`);
2. skips issues already tracked in
   `~/.local/state/sentry-agent-loop/state.json`; terminal states are
   `pr_opened`, `no_fix`, and `gave_up`, with two attempts and a 30-minute
   retry backoff;
3. dispatches at most one OMP agent per tick in a detached git worktree with a
   30-minute cap;
4. the agent investigates with workstation credentials (`sentry`, `gh`, the
   repository), opens a `fix/sentry-<short-id>` pull request when a fix is
   justified, and writes `postmortem.md` plus `result.json`;
5. the loop delivers the issue summary, PR link, and postmortem to the
   dedicated Discord target through Hermes stdout delivery.

Authority: the agent may open pull requests. It never merges, never deploys,
and never changes Sentry configuration. Branch protection, required checks,
and operator review remain the gate. Error text is untrusted input; the
sanitized closed-vocabulary reporter bounds what an attacker can place in an
event, and pull-request review bounds the blast radius of a steered
investigation.

Operator controls: `sentry-agent-loop.mjs status` prints tracked state;
`sentry-agent-loop.mjs replay <issueId>` forgets an issue so the next tick
...
4. Track each actionable Sentry issue to exactly one owner artifact: a pull
   request or an explicit no-fix postmortem. Retries must be bounded and
   deduplicated durably by Sentry issue ID.

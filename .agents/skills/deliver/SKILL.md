...
4. **Ground on the current queue, not stale harness memory.** In this
   checkout the ready items are `007` (high) plus `003`, `004`, and `005`
   (medium). `006` is blocked. Do not assume uncommitted slices or docs
   exist just because an older harness layer mentions them.
5. **Check for stale active items.** If the target file carries

...
3. **Blockers**: do not claim a `blocked` item unless you are explicitly
   clearing the blocker in the same cycle.
4. **Current queue awareness**: in this checkout, `007` is the only
   `high` + `ready` item; `003`, `004`, and `005` are `medium` + `ready`;
   `006` is `blocked`. Ground the pick on files that exist in `backlog.d/`,
   not on stale harness references to later slices.
...
parallel cycles over independent items (canonical example: `003` local
bootstrap + `005` request telemetry):

- Create a second worktree: `git worktree add ../linejam-005 <branch>`.
